...
class Chat extends React.Component {
  ...
  toggle(e) {
    ...
    if (action === 'submit') {
      ...
      if (!checkmode['allow']) {
        ...
        this.props.history.push('delete-chat?id=' + this.state.deleteid)
      }
    }
    this._isMounted &&
      this.setState({
  ...
  async componentDidMount() {
    ...
    const url1 = REACT_APP_RESTAPI + 'modules/chat/chat.php'
    ...
    axios({
      ...
      .then((response) => {
        ...
        itemsarr.forEach(function (item, i) {
          ...
          if (item['attachment']['name'] !== undefined) {
            let filename = App.get_display_filename(item['attachment'], 'thumb')
            itemsarr[i]['disp_attachment'] = Parser(
              '<' +
                'img src="' +
                App.filename_url(filename) +
                '" class="img-fluid img-thumb"  >',
            )
          } else {
            itemsarr[i]['disp_attachment'] = ''
          }
        })
  ...
  render() {
    ...
    const header = [
      ...
      },
      {
        ...
        cell: (row) => (
          <p>
            <a href={'add-chat?id=' + row.id} className="btn btn-xs editlink">
              Edit
            </a>{' '}
            &nbsp;{' '}
            <span
                ...
                <div className="col-xl-12">
                  <section className="box ">
                    <header className="panel_header">
                      <h2 className="title float-left">All Chats</h2>
                    </header>
                    <div className="content-body">
                      ...
                      <div>
                        ...
                        >
                          ...
                          <ModalBody>
                            Are you sure you want to delete chat ID:{' '}
                            {this.state.deleteid} ?
                          </ModalBody>
                          <ModalFooter>
                            <Button
