  ...
  toggle(e) {
    ...
    if (action === 'submit') {
      ...
      if (!checkmode['allow']) {
        ...
        this.props.history.push('delete-client?id=' + this.state.deleteid)
      }
    }
    this._isMounted &&
      this.setState({
      ...
      clientsList.push(
        ...
        >
          <div className="user card">
            <NavLink to={'add-client?id=' + this.props.clients[i].id}>
              <div
                className="user-img contain"
                style={{
                  backgroundImage:
                    'url(' +
                    REACT_APP_RESTAPI +
                    '' +
                    this.props.clients[i].image.name +
                    ')',
                }}
              ></div>
            </NavLink>
            <div className="user-info">
              <NavLink to={'add-client?id=' + this.props.clients[i].id}>
                <h4>{this.props.clients[i].name}</h4>
              </NavLink>
              <p>
                <a
                  href={'' + this.props.clients[i].url}
                  target="_blank"
                  rel="noopener noreferrer"
                >
                  {this.props.clients[i].url}
                </a>
              </p>
              <NavLink
                className="editlink btn btn-xs"
                to={'add-client?id=' + this.props.clients[i].id}
              >
                Edit
              </NavLink>
              <span
                className="text-danger textlink dellink btn btn-xs"
                value={this.props.clients[i].id}
                data-id={this.props.clients[i].id}
                data-title={this.props.clients[i].name}
      ...
      <div className="row">
        ...
        <div>
          ...
          >
            ...
            <ModalBody>
              Are you sure you want to delete client: {this.state.deletetitle} ?
            </ModalBody>
            <ModalFooter>
              <Button
